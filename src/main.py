...
class WineOriginClassifier:
    ...
    def hyperparameter_tuning(self, skip=False):
        ...
        if skip:
            return param_grid["n_estimators"][-1], param_grid["max_depth"][-1]
    ...
    def train_random_forest(self, n_estimators=100, max_depth=None):
        ...
        clf = RandomForestClassifier(n_estimators=n_estimators, max_depth=max_depth, random_state=16)
...
if __name__ == "__main__":
    wine_data_source = "output/wine_quality_features+_1000.csv"

    wine_data = WineOriginClassifier(wine_data_source)
    wine_data.preprocess_data()
